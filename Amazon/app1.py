...
def load_data():
    df= pd.read_csv(r'\AMZN\df.csv', parse_dates= ['Date'], dayfirst= False) 
...
# Load ARIMA model
def load_model():
    return joblib.load(r'\AMZN\arima_final_model.pkl')

model= load_model()


# Input: Number of days to forecast
n_steps= st.number_input("How many future business days to forecast 'Amazon Stock price' from 18-06-25?:", min_value= 1, max_value= 365, value= 30)
